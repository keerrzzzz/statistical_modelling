...
cor_log <- cor(log_velocity, log_current)
cat('\nLog-Log Correlation:', round(cor_log, 4))
cat('\nx and y is a better linear relationship for modelling purposes since the correlation is more close to 1 compare with original (', round(cor_value, 4), ')')
```
...
cat('from the picture log-log is better. since the points are more close to the line')
```
...
cat("error variance σ² =", round(sigma2, 5), "\n")
```

## Supervisors Data Analysis
...
cat('\nSince p_quad=0.1415, simple linear model is sufficient')
...
final_model <- lm(log_supervisors ~ log_workers)
cat("log(Supervisors) = -1.4846 + 0.9092 × log(Workers)\n")
...
cat("Predicted supervisors:", round(pred_original[1], 2), "\n")
cat("95% CI:", round(pred_original[2], 2), "to", round(pred_original[3], 2), "\n")

if(256 >= pred_original[2] && 256 <= pred_original[3]) {
  cat("\n256 is within CI  NOT surprising")
} else {
  cat("\n256 is outside CI  WOULD be surprising")
}
